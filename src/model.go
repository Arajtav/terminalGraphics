...
/*  model stores data in ready to process form
    that means rotation and scale is applied when changing data in model, not when reading from it
    that also means rotation and scale needs to be private, and only changeable by method which will recalculate every Vertex
*/

// Model, stores transformed 3d data, and covers transformations in object space. Position can be changed, because it will be calculated in render time.
type Model struct {
    vertices    []Vertex
...
// Checks if Vertex exists in model. returns 65535 if Vertex doesn't exist. Vertex must be transformed already
func (m *Model) getVertexId(v Vertex) uint16 {
    for i := 0; i<len(m.vertices); i++ {
        if m.vertices[i] == v { return uint16(i) }
    }
    return 65535
}
...
func (m *Model) SetMaterial(mt tc.Material) uint8 {
    for i := 0; i<len(m.materials); i++ {
        if mt == m.materials[i] { return uint8(i) }
    }
